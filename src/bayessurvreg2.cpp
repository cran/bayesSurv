...
// priorb1D[] ............. double parameters for constructor of the random effects object
//    priorb1D[0, ..., nRandom*nCluster-1] = initial values for random effects
...
//    related to the distribiution of the random intercept
//
              ...
              const int* version,      const int* mainSimul,     int* errP)
{
  try{
    ...
    *errP = 0;
    std::string dir = *dirP;
    if (*version != 2 && *version != 3) throw returnR("Error: unimplemented version appeared in bayessurvreg2", 1);
    ...
    /** Object for b1 & D1/random intcpt G-spline  and b2 & D2/random iintcpt G-spline   **/
    ...
    const int nCluster1 = bb1->nCluster();
    const int nCluster2 = bb2->nCluster();
    if (!regresRes1) throw returnR("Not enough memory available in bayessurvreg2 (regresRes1)", 1);
    regresRes_GS(regresRes1, Y1, beta1, bb1->bMP(), X1, bb1->nwithinClP(), &nobs, &nCluster1);
    ...
    if (*doubly){
      ...
      regresRes_GS(regresRes2, Y2, beta2, bb2->bMP(), X2, bb2->nwithinClP(), &nobs, &nCluster2);
    }
      ...
      for (int witer = 1; witer <= nthin; witer++){  /**  thinning cycle  **/
        ...
        case 3:
          ...
          if (bb1->nRandom()){
            ...
            g_bb1->update_alla(mixtureN_b1, a_ipars1_b, &iterTotalNow);
            switch (specif_b[0]){
            case 1:
	    ...
	    break;
          case 3:
            ...
            if (bb2->nRandom()){
              ...
              g_bb2->update_alla(mixtureN_b2, a_ipars2_b, &iterTotalNow);
              switch (specif_b[1]){
              case 1:
      ...
      if ((*mainSimul) || writeAll){
        ...
        case 3:
	      ...
	      l_moments_b, l_lambda1_b, l_log_poster1_b, &nCluster1, &storea_b1P, &ZERO_INT, &storer_b1P, 0, &writeAll, iwork1_b, dwork1_b,
              ...
              _null_weight1_b, out_format[0], out_format[1]);
          }
          if (bb2->nRandom()){
	      ...
	      l_moments_b, l_lambda2_b, l_log_poster2_b, &nCluster2, &storea_b2P, &ZERO_INT, &storer_b2P, 0, &writeAll, iwork2_b, dwork2_b,
              ...
              _null_weight2_b, out_format[0], out_format[1]);
          }
          break;
        }
    ...
    *iterM = iter - 1;
    ...
    /** Write current covariance matrix of random effects and random effects/random intercept G-spline  to initial arrays **/
    switch (*version){
    case 2:
      if (bb1->nRandom()){
